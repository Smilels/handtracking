...
if __name__ == '__main__':
    ...
    min_bbx_dist_thresh = 20
    min_point_dist_thresh = 0.01
    ...
    while True:
        ...
        #                                              detection_graph, sess)
        # check if detect wrong place or human face, how much the score is
        ind = np.argmax(scores)
        bbx = boxes[ind]

        bbx = 0.2 * pre_bbx + 0.8 * bbx
        ...
        if len(points_crop):
            ...
            point_dist = np.sqrt(previous_center_point-center_point)
            if point_dist > min_point_dist_thresh:
                print("esmatied hand is far away from previous hand bbx")
